...
A = pi*(10.1/2).^2;                     % Rocket projected area (m^2)
Gravity = 9.81;                         % Gravity (m/s^2)
Launch_Rod_Length = 0;                  % Length of launch rod (m)
Mass_Rocket_With_Motor = 2970000;       % Mass with motor (kg)
Mass_Rocket_Without_Motor = (2970000-2290000);     % Mass without motor (kg)
...
% Figure 1
b=animatedline(x(1:n),y(1:n),'Color','r');
subplot(3,3,1) 
plot(x(1:n),y(1:n));
xlim([0 inf]);
ylim([0 inf]);
...
grid on
c=animatedline(t(1:n),Vx(1:n),'Color','r');
...
grid on
d=animatedline(t(1:n),Vy(1:n),'Color','r');
...
title({'Theta'});
grid on
an=animatedline(t(1:n),Theta(1:n),'Color','r');
...
grid on
f=animatedline(Distance(1:n),Theta(1:n),'Color','r');
...
title({'Rocket Mass'});
grid on
g=animatedline(t(1:n),Mass(1:n),'Color','b');
...
title({'Thrust'});
grid on
h=animatedline(t(1:n),Thrust(1:n),'Color','r');
...
title({'Drag Force'});
grid on
i=animatedline(t(1:n),Drag(1:n),'Color','r');
...
title({'Normal Force'});
grid on
j=animatedline(Distance(1:n),Fn(1:n),'Color','r');
